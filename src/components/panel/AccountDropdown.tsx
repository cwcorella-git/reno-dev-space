  ...
  return (
    <>
      <div className="relative" ref={dropdownRef}>
        ...
        {/* Dropdown */}
        {isOpen && (
          <div className="absolute bottom-full right-0 mb-2 w-72 bg-gray-900 border border-white/20 rounded-xl shadow-2xl overflow-hidden max-h-[80vh] overflow-y-auto">
            {/* Status message */}
            {statusMessage && (
            ...
            {!confirmAction && (
              <>
                ...
                <div className="py-1 border-b border-white/10">
                  <button onClick={() => setConfirmAction('votes')} className="w-full px-4 py-2 text-left text-sm text-gray-300 hover:bg-white/5">Clear My Votes</button>
                  <button onClick={() => setConfirmAction('content')} className="w-full px-4 py-2 text-left text-sm text-gray-300 hover:bg-white/5">Delete My Content</button>
                  <button onClick={() => setConfirmAction('account')} className="w-full px-4 py-2 text-left text-sm text-red-400 hover:bg-white/5">Delete Account</button>
                </div>
                  ...
                  <>
                    ...
                    <div className="p-3 space-y-3 border-b border-white/10">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <input type="number" value={goalInput} onChange={(e) => setGoalInput(e.target.value)} className="w-16 px-2 py-1 bg-white/10 border border-white/20 rounded text-white text-xs" />
                      ...
                      <button
                        onClick={() => { setShowContentModal(true); setIsOpen(false) }}
                        className="w-full px-3 py-2 bg-amber-600/20 hover:bg-amber-600/30 text-amber-300 rounded text-sm font-medium flex items-center gap-2"
                      >
                ...
                {/* Sign out */}
                <button onClick={handleLogout} className="w-full px-4 py-2.5 text-left text-sm text-red-400 hover:bg-white/5 flex items-center gap-2">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
